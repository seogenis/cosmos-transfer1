...
def process_segmentation_files(segmentation_dir, output_dir, viz_dir, video_path=None, weights_dict=None, setting_name='setting1', robot_keywords=None):
    ...
    if png_files:
        ...
        for i, (json_file, png_file) in enumerate(zip(json_files, png_files)):
            ...
            vis_mask = np.where(pixel_is_robot_2d, 
                               weights_dict['vis']['foreground'], 
                               weights_dict['vis']['background'])
            
            edge_mask = np.where(pixel_is_robot_2d, 
                                weights_dict['edge']['foreground'], 
                                weights_dict['edge']['background'])
            
            seg_mask = np.where(pixel_is_robot_2d, 
                                       weights_dict['seg']['foreground'], 
                                       weights_dict['seg']['background'])
        ...
        try:
            ...
            for i, json_file in enumerate(tqdm(json_files[:num_frames], desc="Processing frames")):
                ...
                # Create masks based on the is_robot classification
                depth_mask = np.where(pixel_is_robot_2d, weights_dict['depth']['foreground'], weights_dict['depth']['background'])
                vis_mask = np.where(pixel_is_robot_2d, weights_dict['vis']['foreground'], weights_dict['vis']['background'])
                edge_mask = np.where(pixel_is_robot_2d, weights_dict['edge']['foreground'], weights_dict['edge']['background'])
